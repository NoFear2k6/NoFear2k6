

## 👨‍💻 基本情報

| 項目 | 内容 |
|------|------|
| 生年月日 | 2006年12月18日（19歳） |
| 大学 | フォンドン大学（Phương Đông University） |
| 学部・プログラム | システムエンジニア育成プログラム（日本語特化） |
| 専攻 | ソフトウェア工学（K24・2024〜2028年入学） |
| 日本語能力 | JLPT N4 勉強中 |
| メール | tu.524100123@pduni.edu.vn/ lengoctu3000@gmail.com |
| 電話 | 0987-974-323 |
| 居住地 | ベトナム・タインホア省・バンソン社 |

## 💡 自己紹介

フォンドン大学情報技術学部の2年生です。  
現在、HTML、CSS、JavaScript、PHP、C++、SQL Server を中心に学習しており、日本語は JLPT N4 レベルを勉強中です。  
基本的なフロントエンド・バックエンド開発の知識を身につけながら、実践的なプロジェクトを通してスキル向上に取り組んでいます。

---

## 🛠 主なプロジェクト経験

### ■ グループプロジェクト（2024）
クラスメイトと協力し、「ケーキ販売ウェブサイト」を開発しました。  
担当範囲は以下の通りです。  
- 商品一覧ページの実装  
- 詳細ページの作成  
- 検索機能の実装  
**使用技術:** HTML / CSS / JavaScript  

### ■ 個人開発（2025）
中古コピー機販売サイト「Photocopy Machine」をフルスクラッチで開発しました。  
- レスポンシブデザイン対応  
- 商品登録・表示・検索機能の実装  
- SQL Server を用いたデータ管理  
- フロントエンドおよびバックエンドの全工程を担当  
**使用技術:** HTML / CSS / Bootstrap / PHP / SQL Server  
GitHub リンク: https://github.com/NoFear2k6/NoFear2k6

---

## 🎯 将来の目標

将来的には、日本でエンジニアとして働きたいと考えています。  
そのために、以下の目標を掲げています。

- 日本の最新IT技術・開発手法を学ぶこと  
- 実務に対応できる堅実なウェブ開発スキルを習得すること  
- ベトナムと日本のIT分野の架け橋として活躍できるエンジニアになること  

---

## 📚 現在の学習分野

- フロントエンド：HTML / CSS / JavaScript  
- バックエンド：PHP / SQL Server  
- その他：C++、Git、基本的なデータベース操作  
- 日本語：JLPT N4（学習継続中）  

---



## ソフトスキル
![https://img.shields.io/badge/チームワーク-重要-blue]
![https://img.shields.io/badge/コミュニケーション-良好-green]
![https://img.shields.io/badge/向上心 -得意-yellow]
![https://img.shields.io/badge/忍耐力-向上中-orange]
![https://img.shields.io/badge/協調性-実践中-red]

---




## 📚 学習・資格状況

| 期間 | 内容 |
|------|------|
| 2024.09 ～ 2028.07予定 | フォンドン大学・情報技術学部 |
| 2025 | 個人ウェブ開発プロジェクト完成 |
| 現在 | JLPT N4 取得に向けて勉強中 |

---

## ✨ 趣味・好きなこと

- 映画鑑賞（特にSF・アニメ）
- 音楽を聴くこと
- 新しい技術を試すこと

---

## 📬 連絡先・リンク

[![Email](https://img.shields.io/badge/Email-ff4b4b?style=for-the-badge&logo=gmail&logoColor=white)](mailto:lengoctu3000@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/NoFear2k6)

---

いつも新しいことに挑戦し続けています！  
日本の企業・チームで一緒に働ける日を楽しみにしています。  
お気軽にメッセージください！よろしくお願いします 🙇‍♂️


## 📬 連絡先

- Email: tu.524100123@pduni.edu.vn
- 居住地: ベトナム・タインホア省・バンソン社

## 👨‍💻 基本情報

| 項目 | 内容 |
|------|------|
| 生年月日 | 2006年12月18日（19歳） |
| 大学 | フォンドン大学（Phương Đông University） |
| 学部・プログラム | システムエンジニア育成プログラム（日本語特化） |
| 専攻 | ソフトウェア工学（K24・2024〜2028年入学） |
| 日本語能力 | JLPT N4 勉強中 |
| メール | tu.524100123@pduni.edu.vn/ lengoctu3000@gmail.com |
| 電話 | 0987-974-323 |
| 居住地 | ベトナム・タインホア省・バンソン社 |

## 💡 自己紹介

フォンドン大学情報技術学部の2年生です。  
現在、HTML、CSS、JavaScript、PHP、C++、SQL Server を中心に学習しており、日本語は JLPT N4 レベルを勉強中です。  
基本的なフロントエンド・バックエンド開発の知識を身につけながら、実践的なプロジェクトを通してスキル向上に取り組んでいます。

---

## 🛠 主なプロジェクト経験

### ■ グループプロジェクト（2024）
クラスメイトと協力し、「ケーキ販売ウェブサイト」を開発しました。  
担当範囲は以下の通りです。  
- 商品一覧ページの実装  
- 詳細ページの作成  
- 検索機能の実装  
**使用技術:** HTML / CSS / JavaScript  

### ■ 個人開発（2025）
中古コピー機販売サイト「Photocopy Machine」をフルスクラッチで開発しました。  
- レスポンシブデザイン対応  
- 商品登録・表示・検索機能の実装  
- SQL Server を用いたデータ管理  
- フロントエンドおよびバックエンドの全工程を担当  
**使用技術:** HTML / CSS / Bootstrap / PHP / SQL Server  
GitHub リンク: https://github.com/NoFear2k6/NoFear2k6

---

## 🎯 将来の目標

将来的には、日本でエンジニアとして働きたいと考えています。  
そのために、以下の目標を掲げています。

- 日本の最新IT技術・開発手法を学ぶこと  
- 実務に対応できる堅実なウェブ開発スキルを習得すること  
- ベトナムと日本のIT分野の架け橋として活躍できるエンジニアになること  

---

## 📚 現在の学習分野

- フロントエンド：HTML / CSS / JavaScript  
- バックエンド：PHP / SQL Server  
- その他：C++、Git、基本的なデータベース操作  
- 日本語：JLPT N4（学習継続中）  

---



## ソフトスキル
!https://img.shields.io/badge/チームワーク-重要-blue
!https://img.shields.io/badge/コミュニケーション-良好-green
!https://img.shields.io/badge/向上心 -得意-yellow
!https://img.shields.io/badge/忍耐力-向上中-orange
!https://img.shields.io/badge/協調性-実践中-red

---




## 📚 学習・資格状況

| 期間 | 内容 |
|------|------|
| 2024.09 ～ 2028.07予定 | フォンドン大学・情報技術学部 |
| 2025 | 個人ウェブ開発プロジェクト完成 |
| 現在 | JLPT N4 取得に向けて勉強中 |

---

## ✨ 趣味・好きなこと

- 映画鑑賞（特にSF・アニメ）
- 音楽を聴くこと
- 新しい技術を試すこと

---

## 📬 連絡先・リンク

[![Email](https://img.shields.io/badge/Email-ff4b4b?style=for-the-badge&logo=gmail&logoColor=white)](mailto:lengoctu3000@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/NoFear2k6)

---

いつも新しいことに挑戦し続けています！  
日本の企業・チームで一緒に働ける日を楽しみにしています。  
お気軽にメッセージください！よろしくお願いします 🙇‍♂️


## 📬 連絡先

- Email: tu.524100123@pduni.edu.vn / lengoctu3000@gmail.com
- 居住地: ベトナム・タインホア省・バンソン社
