<p align="center">
  <img src="https://raw.githubusercontent.com/NoFear2k6/NoFear2k6/main/header.svg" />
</p>


<p align="center">
  <img src="https://raw.githubusercontent.com/NoFear2k6/NoFear2k6/main/cyberpunk.svg" />
</p>






## 👨‍💻 基本情報

| 項目 | 内容 |
|------|------|
| 生年月日 | 2006年12月18日（19歳） |
| 大学 | フォンドン大学（Phương Đông University） |
| 学部・プログラム | システムエンジニア育成プログラム（日本語特化） |
| 専攻 | ソフトウェア工学（K24・2024〜2028年入学） |
| 日本語能力 | JLPT N4 勉強中 |
| メール | tu.524100123@pduni.edu.vn/ lengoctu3000@gmail.com |
| 電話 | 0987-974-323 |
| 居住地 | ベトナム・タインホア省・バンソン社 |

💡 自己紹介

フォンドン大学情報技術学部の2年生です。
現在、HTML、CSS、JavaScript、PHP、C++、SQL Server などのプログラミング言語・技術を学びながら、JLPT N4 レベルの日本語を勉強しています。
実践的なプロジェクトに参加することで、ソフトウェア開発の基礎知識を身につけるとともに、プログラミングスキルの向上に努めています。

🛠 主なプロジェクト経験
 グループプロジェクト（2024）

クラスメイトと協力し、ホテル管理ウェブサイトを開発しました。
担当した業務は以下の通りです：

ウェブサイトのUIデザイン

検索機能の実装

使用技術: HTML / CSS / JavaScript

■ 個人開発（2025）

「Lords Mobile Tips」というゲーム攻略サイトをフルスクラッチで開発しました。
実施した作業は以下の通りです：

レスポンシブデザインの実装

記事投稿・表示・検索機能の開発

フロントエンドおよびバックエンド全体の設計と実装を担当

使用技術: HTML / CSS / Bootstrap / PHP / SQL Server

GitHub: https://github.com/NoFear2k6/NoFear2k6

🎯 将来の目標

将来的には、日本でITエンジニアとして働きたいと考えています。
そのために、次の目標を掲げています。

日本の最新技術や開発手法を学ぶこと

堅実なプログラミングスキルおよびコミュニケーション能力を身につけること

ベトナムと日本のIT分野をつなぐ架け橋となるエンジニアになること

📚 現在の学習分野

フロントエンド：HTML / CSS / JavaScript

バックエンド：PHP / SQL Server

その他：C++、Git、基本的なデータベース操作

日本語：JLPT N4（学習継続中）

 <p align="center">
  <img src="https://raw.githubusercontent.com/NoFear2k6/NoFear2k6/main/jp-level.svg" />
</p>


## ソフトスキル

![Teamwork](https://img.shields.io/badge/チームワーク-重要-blue)
![Communication](https://img.shields.io/badge/コミュニケーション-良好-green)
![Effort](https://img.shields.io/badge/向上心-得意-yellow)
![Patience](https://img.shields.io/badge/忍耐力-向上中-orange)
![Cooperation](https://img.shields.io/badge/協調性-実践中-red)

---




## 📚 学習・資格状況

| 期間 | 内容 |
|------|------|
| 2024.09 ～ 2028.07予定 | フォンドン大学・情報技術学部 |
| 2025 | 個人ウェブ開発プロジェクト完成 |
| 現在 | JLPT N4 取得に向けて勉強中 |

---

## ✨ 趣味・好きなこと

- ボクシング
- 音楽を聞くこと（とくに英語の曲）
- バドミントン
- 英語を勉強すること

---

## 📬 連絡先・リンク

[![Email](https://img.shields.io/badge/Email-ff4b4b?style=for-the-badge&logo=gmail&logoColor=white)](mailto:lengoctu3000@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/NoFear2k6)
[![Facebook](https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white)](https://www.facebook.com/le.ngoc.tu.462229)

---

いつも新しいことに挑戦し続けています！  
日本の企業・チームで一緒に働ける日を楽しみにしています。  
お気軽にメッセージください！よろしくお願いします。


